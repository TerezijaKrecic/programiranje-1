...
(* KAKO ZAGANJAMO s CTRL+SHIFT+P, RUN TASK, OCAML *)
(* oz. sama si definirala bližnjico: Shift+R = Run OCaml; Shift+T = Restart OCaml *)
let f x =
  let a = x + x in
  let b = x * x in
  a * b

(* funkcija, ki vrne dolžino seznama *)
...
let middle_of_triple = function
  ...
  *)
  ...
  (* ALI
  let middle_of_triple (prvi, drugi, tretji) = drugi
  *)

  (* ALI
  let middle_of_triple triple = match triple with
    | (_, y, _) -> y
  *)
